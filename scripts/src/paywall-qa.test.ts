...
const { canAccessProject } = await import("../../artifacts/api-server/src/services/projects/projectAccess");
...
function req(accessToken?: string, adminToken?: string) {
  return {
    query: accessToken ? { accessToken } : {},
    get(name: string) {
      if (name.toLowerCase() === "authorization" && adminToken) return `Bearer ${adminToken}`;
      return undefined;
    },
  };
}
...
run("accessToken is required for guest project access", () => {
  assert.equal(canAccessProject(req() as any, project as any), false);
  assert.equal(canAccessProject(req("wrong") as any, project as any), false);
  assert.equal(canAccessProject(req("project-secret") as any, project as any), true);
  assert.equal(canAccessProject(req(undefined, "admin-test-token") as any, project as any), true);
});
