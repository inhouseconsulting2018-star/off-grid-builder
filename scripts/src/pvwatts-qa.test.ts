...
await run("California residential property uses PVWatts roof mount production", async () => {
  ...
  assert.equal(lastPvwattsUrl?.searchParams.get("dc_ac_ratio") != null, true);
});
...
await run("Invalid geocode falls back to approximate state assumptions without crashing", async () => {
  ...
  assert.match(result.productionEstimateLabel, /Approximate/);
  assert.ok(result.notes.some((note: string) => note.includes("PVWatts production data was unavailable")));
});
